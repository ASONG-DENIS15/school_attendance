    ...
    if ($action === 'add_user') {
        ...
        if ($name === '' || $email === '' || $password === '' || $role === '') {
            ...
            if ($existing) {
                ...
                db_query($conn, 'INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)', [$name, $email, hash_password($password), $role]);
...
$logs = db_fetch_all($conn, 'SELECT id, created_at, action, user_name FROM system_logs ORDER BY created_at DESC LIMIT 30');
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>System Administrator Portal</title>
...
<body>
  <div class="sidebar">
    <h2>System Admin</h2>
    <a href="systemadmin.php" class="active"><i class="fas fa-file-alt"></i> System Logs</a>
    <a href="admin.php"><i class="fas fa-users"></i> User Management</a>
